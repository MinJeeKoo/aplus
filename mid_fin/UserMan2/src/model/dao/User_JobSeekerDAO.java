...
public class User_JobSeekerDAO {
	...
	public User_JobSeeker findUser(String userId) throws SQLException {
		...
		try {
			...
			if (rs.next()) {						// 학생 정보 발견
				User_JobSeeker user = new User_JobSeeker(		// User 객체를 생성하여 학생 정보를 저장
					userId,
	...
	public List<User_JobSeeker> findUserList() throws SQLException {
		...
		try {
			ResultSet rs = jdbcUtil.executeQuery();			// query 실행			
			List<User_JobSeeker> userList = new ArrayList<User_JobSeeker>();	// User들의 리스트 생성
			while (rs.next()) {
				User_JobSeeker user = new User_JobSeeker(			// User 객체를 생성하여 현재 행의 정보를 저장
					rs.getString("userId"),
					rs.getString("password"),
					rs.getString("name"),
					rs.getString("school"),
